...
$totalLessons       = $model->countTeacherLessons($teacherId);
$totalAssignments   = $model->countTeacherAssignments($teacherId);
$totalActivities    = $model->countTeacherActivities($teacherId);
$totalQuizzes       = $model->countTeacherQuizzes($teacherId);

$latestLessons      = $model->latestLessons($teacherId,5);
$latestAssignments  = $model->latestAssignments($teacherId,5);
$latestActivities   = $model->latestActivities($teacherId,5);
$latestQuizzes      = $model->latestQuizzes($teacherId,5);

$lessonChart        = $model->monthlyLessons($teacherId);
$assignmentChart    = $model->monthlyAssignments($teacherId);
$activityChart      = $model->monthlyActivities($teacherId);
$quizChart          = $model->monthlyQuizzes($teacherId);

$courseChart        = $model->lessonsByCourse($teacherId);
$classChart         = $model->studentsByClass($teacherId);
$pieChart           = $model->activitiesVsQuizzes($teacherId);
$yearActivity       = $model->teacherActivityYearly($teacherId);
...
document.getElementById('teacherActivityChart'),
...
data:<?= json_encode(array_values($lessonData)) ?>,
...
data:<?= json_encode(array_values($assignmentData)) ?>,
...
data:<?= json_encode(array_values($quizData)) ?>,
